...
| **曲調註記** | 見下一節 |
| **歌詞字級** | `最小` ～ `最大` 五段（以「中」為基準，每段 ±4px）。選擇會被記住，換頁與下次開啟都維持，投影時很方便 |
...
> **這是「記錄」功能，不是「切換版本」功能。** 按下去不會換成別的音檔，而是把目前這個人聲音檔屬於哪種版本記錄下來，讓後面的人一看就知道。若您熟悉某首詩歌，歡迎順手幫忙記錄。
...
|------|----|------|
| 1 | 6 | 四部合唱 |
| 2 | **90–99** | **自訂分類**（聚會、詩班錄音、練唱版⋯，多個之間依 id 排序） |
| 3 | 4 | 人聲 |
| 4 | 3 | 鋼琴 |
| 5+ | 7–10 / 11–14 | 四部合唱 1–4 部 / 合唱 1–4 部（依 id 升冪） |

- **排在第一個的版本就是預設播放的版本**。「有四部合唱＝標準版」的自動判定也是基於這個順序
- **自訂分類請用 90–99**：官方目前只用到 3–14，這個區段不會撞號；程式判斷的是「id ≥ 90」而非特定數字，新增分類不必改程式。建議從 91 開始依序編
...
| 歌詞字級基準 | `body.desktop .vt` / `.chorus p`（電腦版基準 32px）與非 desktop 的 `.vt` / `.chorus p`；使用者的加減值由 CSS 變數 `--lyr-adj` 套用 |
| 歌詞字級選項 | JS 中的 `FS_STEPS`（目前 −8／−4／0／+4／+8） |
